...
export default {
  ...
  methods: {
    async fetchProduct() {
      ...
      this.error = ''
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL || (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:8000/api' : 'https://ecommerce-backend-qqda.onrender.com/api')}/productos/${id}`)
    ...
    async addToCart() {
      ...
      try {
        ...
        if (!cartId) {
          const { data } = await axios.post((import.meta.env.VITE_API_URL || (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:8000/api' : 'https://ecommerce-backend-qqda.onrender.com/api')) + '/carritos', {
        ...
        await axios.post(`${import.meta.env.VITE_API_URL || (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:8000/api' : 'https://ecommerce-backend-qqda.onrender.com/api')}/carritos/${cartId}/items`, {
